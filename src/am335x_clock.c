...
static inline void enable_module(volatile uint32_t* ctrl) {
  *ctrl = 0x02000000;
  while ((*ctrl & 0x03030000) != 0x02000000) continue;
}
static inline void wkup_module(volatile uint32_t* ctrl) {
  *ctrl = 0x02000000;
  while ((*ctrl & 0x03000000) != 0x02000000) continue;
}
...
static inline void bypass_pll(volatile uint32_t* ctrl,
  ...
  *ctrl = (*ctrl & ~0x07000000) | 0x04000000;
  while ((*idlest & 0x01010000) == 0x01000000) continue;
  return;
}
...
static inline void lock_pll(volatile uint32_t* ctrl,
                            volatile uint32_t* idlest) {
  *ctrl |= 0x07000000;
  while ((*idlest & 0x01010000) == 0x00010000) continue;
  return;
}
...
void am335x_clock_init_per_pll(void) {
  /*DPLL in bypass*/
...
void am335x_clock_enable_l3_l4wkup(void) {
  ...
  // wait until region are waked-up
